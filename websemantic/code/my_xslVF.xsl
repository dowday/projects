...
>
    ...
    <!--    ↓ Plant cover  -->
        ...
        <!--    ↓ City Furniture -->
        <xsl:for-each select="city:cityObjectMember/frn:CityFurniture/frn:lod3Geometry/gml:MultiSurface/gml:surfaceMember/gml:Polygon/gml:exterior/gml:LinearRing">
			<shape>
				<IndexedFaceSet>
					<xsl:variable name="vpoint" select="gml:posList"/>
					<xsl:variable name="nbelt" select="string-length(normalize-space(gml:posList))
				...
				<Appearance>
						 <Material diffuseColor="#CDAF9A"></Material>
				</Appearance>
			</shape>
    </xsl:for-each>
        <!--    ↓ Land Use  -->
        <xsl:for-each select="city:cityObjectMember/luse:LandUse/luse:lod3MultiSurface/gml:MultiSurface/gml:surfaceMember/gml:Polygon/gml:exterior/gml:LinearRing">
			<shape>
				<IndexedFaceSet>
					<xsl:variable name="vpoint" select="gml:posList"/>
					<xsl:variable name="nbelt" select="string-length(normalize-space(gml:posList))
				...
				<Appearance>
					<Material diffuseColor="#8F4700"></Material>
				</Appearance>
			</shape>
		</xsl:for-each>
        ...
        <shape>
            <IndexedFaceSet>
                ...
                <xsl:attribute name = "solid" ><xsl:value-of select = "false()"/></xsl:attribute>
                <xsl:attribute name="coordindex"><xsl:value-of select="0 to ($sqnbelt)" separator=" "/></xsl:attribute>

                <Coordinate>
                    <xsl:attribute name= "point" ><xsl:value-of select = "$vpoint" /></xsl:attribute>
                </Coordinate>
            </IndexedFaceSet>
            <Appearance> <Material diffuseColor="#BA5D1B">  </Material> </Appearance>
        </shape>
    </xsl:for-each>
        ...
        <xsl:for-each select="city:cityObjectMember/gen:GenericCityObject/gen:lod3Geometry/gml:MultiSurface/gml:surfaceMember/gml:Polygon/gml:exterior/gml:LinearRing">
			<shape>
				...
				<Appearance>
						 <Material diffuseColor="#FCD769"></Material>
				</Appearance>
			</shape>
    </xsl:for-each>
