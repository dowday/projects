...
>
    ...
    <xsl:template match="city:CityModel">  <!-- text nodes are ignored -->
        <!--    ↓ Plant cover  -->
        <xsl:for-each select="city:cityObjectMember/veg:PlantCover[contains(Graben-Aussen,Graben-Aussen)]/veg:lod3MultiSurface/gml:MultiSurface/gml:surfaceMember/gml:Polygon/gml:exterior/gml:LinearRing">
            <shape>
                <IndexedFaceSet>
